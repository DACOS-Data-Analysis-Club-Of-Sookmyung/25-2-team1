...
def build_ctx(workdir: Path, spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profitability_components_table = render_T1_YOY(
        metric_rows,
        ["NET_INCOME", "OP_PROFIT", "TAX_EXP", "TOTAL_ASSETS", "EQUITY"],
    )

    profitability_ratio_table = render_T_SIMPLE(metric_rows, ["roa", "roe", "roc"])

    bench = render_T2_BENCH(metric_rows, ["roa", "roe", "roc"])
    bench_profitability_table = bench if bench is not None else "제공 없음"

    return {
        "corp_name": meta.get("corp_name") or meta.get("corp_name_kr") or meta.get("corp_code"),
        "bsns_year": meta.get("bsns_year"),
        "profitability_components_table": profitability_components_table,
        "profitability_ratio_table": profitability_ratio_table,
        "bench_profitability_table": bench_profitability_table,
        "profitability_evidence": pack_evidence(evidence_rows, topk=len(evidence_rows), include_tables=False),
    }
