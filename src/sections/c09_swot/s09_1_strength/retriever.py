...
def _split_strength(metric_rows: Dict[str, Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    core, aux = [], []
...
def build_ctx(workdir: Path, spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "strength_core_table": _md_table("핵심 Strength(AND)", core),
        "strength_aux_table": _md_table("보조 Strength(OR)", aux),
    }
